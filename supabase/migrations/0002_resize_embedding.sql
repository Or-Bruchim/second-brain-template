-- Migration 0002: resize embedding 1024-dim to 384-dim
-- Switching from Cloudflare Workers AI (bge-m3, 1024-dim) to local
-- @xenova/transformers (multilingual-e5-small, 384-dim, no API key needed).
-- chunks is a derived index — safe to rebuild from scratch.

-- Drop dependents
drop index if exists chunks_embedding_hnsw;
drop function if exists hybrid_search(text, vector, int, int);

-- Resize column (pgvector requires drop+add)
alter table chunks drop column if exists embedding;
alter table chunks add column embedding vector(384);

-- Recreate HNSW index for 384-dim
create index chunks_embedding_hnsw
  on chunks using hnsw (embedding vector_cosine_ops)
  with (m = 16, ef_construction = 64);

-- Recreate hybrid_search for 384-dim
